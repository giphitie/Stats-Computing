...
## Empirical Values from inverse cdf ad ratio

inverse_cdf = inv_cdf_cauc(1000)

ratio_of_stan_normals = ratio_of_norms_cauc(1000)

cauchy_samples <- list(
  inverse = inv_cdf_cauc(1000),
  ratio = ratio_of_norms_cauc(1000)
)
...
# Define the target density function (unnormalized standard normal)
f_x <- exp(-x^2 / 2)

# Define the envelope density function (unnormalized standard Cauchy)
g_x <- 1 / (1 + x^2)

# Calculate the ratio M
M <- 2 / sqrt(exp(1))  

# Acceptance probability calculation
acceptance_prob <- f_x / g_x

# Generate uniform random numbers to compare 
u <- runif(n)

# Accept if u <= acceptance_prob / M 

accepted <- u <= (acceptance_prob / M)

# Calculate the empirical proportion of acceptances
